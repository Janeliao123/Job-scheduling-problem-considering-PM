...
Our baseline method is the periodic maintenance method. Periodic maintenance method is cited from Cassady and Kutanoglu (2015), where we calculate a $τ^*$ that could have the largest steady-state machine availability. Steady-state machine availability, named $A(τ)$ is defined below. Based on the $A(τ)$, we differentiate the function to find the $τ^*$ that maximize $A(τ)$. After finding $τ$, we only implement a PM only when the age of the machine is larger than $τ$. 
...
For the first parameter of Weibull distribution, $\eta$, we can see that the total number of PM is much larger when $\eta$ is smaller. This is because $\eta$ control the expected number of failure $m(T)$. Since $\eta$ is the denominator in the function, the expected number of failure $m(T)$ is higher when $\eta$ is smaller. Therefore, it makes sense to plan more PM when $\eta$ is smaller. 
The second parameter of Weibull distribution is $\beta$. We can see that there is no big difference between the total number of PM when $\beta$ changes. However, when $\beta$ is larger, our method tends to set more PM in the later position. This is because $\beta$ controls the distribution of $m(T)$ like Figure 1. When $\beta$ is larger, the value of $m(T)$ increases slowly first but increases tremendously afterwards. As a result, it makes sense to set more PM behind when $\beta$ is larger. 
